...
async function renderPrediction() {
    ...
    if (newData === null) {
        ...
        let peerPredictions = newData.data

        if (peerPredictions.length > 0) {
            peerPredictions.forEach((prediction) => {
                try {
                    document.getElementById("stats").innerHTML += "confidence: " + prediction.faceInViewConfidence.toFixed(4);
                } catch (err) {
                    document.getElementById("stats").innerHTML = err.message;
...
function muteTrack(track){
    ...
    if(muted[track]){
        console.log(`track ${track} already muted`)
        return
    }
